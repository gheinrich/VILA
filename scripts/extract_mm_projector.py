...
if __name__ == "__main__":
    ...
    try:
        model_indices = json.load(
            open(os.path.join(args.model_name_or_path, "pytorch_model.bin.index.json"))
        )
        ...
        v = "pytorch_model.bin"
        for k in torch.load(
            os.path.join(args.model_name_or_path, v), map_location="cpu"
        ).keys():
            if any(key_match in k for key_match in keys_to_match):
                ckpt_to_key[v].append(k)
    ...
    for ckpt_name, weight_keys in ckpt_to_key.items():
        ckpt = torch.load(
            os.path.join(args.model_name_or_path, ckpt_name), map_location="cpu"
        )
