...
class RefCOCODataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        image = Image.open(os.path.join(self.image_folder, '_'.join(image_file.split('_')[:-1]) + '.jpg')).convert('RGB')
...
# DataLoader
def create_data_loader(loaded_data, image_folder, tokenizer, image_processor, model_config, conv_mode, batch_size=1, num_workers=8):
    assert batch_size == 1, "batch_size must be 1"
    dataset = RefCOCODataset(loaded_data, image_folder, tokenizer, image_processor, model_config, conv_mode)
    data_loader = DataLoader(dataset, batch_size=batch_size, num_workers=num_workers, collate_fn=collate_fn, shuffle=False)
    return data_loader
...
def eval_model(args):
    ...
    if args.chunk_idx == 0 and args.use_wandb.lower() == "true":
        print_rank0(args.chunk_idx, 'wandb inititialize with group: {}, project: {}, name: {}\n'.format(args.group, args.project, args.run_name), flush=True)
    ...
    print_rank0('model path: {}\nmodel name: {}\nmodel base: {}\n'.format(model_path, model_name, args.model_base), flush=True)
    ...
    for dataset in eval_dict.keys():
        for split in eval_dict[dataset]:
            ...
            with open(osp.join(args.data_path, "{}_{}.json".format(dataset, split)), 'r') as f:
                loaded_data_full = json.load(f)
            
            loaded_data = get_chunk(loaded_data_full, args.num_chunks, args.chunk_idx)
            data_loader = create_data_loader(loaded_data, args.image_folder, tokenizer, image_processor, model.config, args.conv_mode)
            print_rank0(args.chunk_idx, 'dataset size: {}/{}\n'.format(len(loaded_data), len(loaded_data_full)))
            ...
            for (input_ids, image_tensor, image_file, raw_data, raw_image) in tqdm(data_loader, total=len(loaded_data)):
                ...
                if count < 20 and ans is not None:
                    print('answer: {}\nimage size: {}\n'.format(ans, image_tensor.shape), flush=True)
                count += 1 
            ...
            for i in range(args.num_resample):
                ...
                data_loader = create_data_loader(resamples, args.image_folder, tokenizer, image_processor, model.config, args.conv_mode)
                resamples = []
            ...
            ans_file.close()
            print('answers saved at {}'.format(answers_file))
