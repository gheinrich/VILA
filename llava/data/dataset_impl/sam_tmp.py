...
from llava.mm_utils import is_gemma_tokenizer, tokenizer_image_token, process_image
...
class LazySAMTmpWebDataset(Dataset):
    ...
    def __init__(
        ...
        self.dataset = VILAWebDataset(
            ...
            meta_path="/home/yunhaof/workspace/datasets/sam/reformat_subset/wids-0to255-meta.json"
        )

        # None: use original caption
        # Folder path: use original caption
        # Efficient-Large-Model/sam-recap-VILA-13b
        self.caption_choice = "/home/yunhaof/workspace/datasets/data_synthesis/sam/paper_comparison/spatial_specialist"
        ...
        self.annotation_dir = data_args.meta_path
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx in range(begin_idx, end_idx):
            ...
            url = osp.join("/lustre/fsw/portfolios/nvr/projects/nvr_elm_llm/dataset/sam-reformat", shard.split("/")[-1], shard_key)

            tar_name = osp.relpath(osp.realpath(shard), osp.realpath(self.data_path))
            tar_name = tar_name.split("/")[-1]
            ...
            try:
                if self.data_args.use_refined_caption:
                    refined_caption_file = osp.join(self.annotation_dir, tar_name + ".json")
                    if os.path.exists(refined_caption_file):
                        caption_dict = load(refined_caption_file)
                        caption = caption_dict[url]["output"]
                    else:
