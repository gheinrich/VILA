...
class LazyImageTextWebDataset(Dataset):
    ...
    def __init__(
        ...
        if data_args.start_idx >= 0 and data_args.end_idx >= 0:
            ...
            print(
                "loading subset from {} to {}, total {}".format(
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        for idx in range(begin_idx, end_idx):
            ...
            if self.caption_choice is not None:
                # load new captions
                shard_json_path = osp.join(self.caption_choice, 
                    tar_name + ".json"
                )
                ...
                try:
                    ...
                    if shard_data_path not in self.missing_key:
                        ...
                        print(
                            f"{shard_data_path} not in caption. fallback to original caption temporarially"
                        )

            if caption is None:
                caption = ""
                print(
                    f"{url} not found in {self.caption_choice} or {self.caption_choice_2} "
                )
