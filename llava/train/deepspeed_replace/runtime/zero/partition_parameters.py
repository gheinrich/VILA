...
class AllGatherCoalescedHandle:
    ...
    def wait(self) -> None:
        ...
        for param in self.params:
            ...
            if self.use_secondary_tensor:
                ds_tensor_numel = param.ds_secondary_tensor.ds_numel
...
class Init(InsertPostInitMethodToModuleSubClasses):
    ...
    def _convert_to_deepspeed_param(self, param):
        ...
        def _all_gather_dtype(dtype, params, world_size, rank_in_group, ds_process_group):
            ...
            if use_secondary_tensor:
                ...
                partition_sz = sum(p.ds_secondary_tensor.ds_numel for p in params)
            ...
            assert partitions[rank_in_group].numel() * world_size == flat_tensor.numel() 
    ...
    def _aligned_size_sec(self, param):
        return param.ds_numel + self._padding_size_sec(param)

    def _padding_size_sec(self, param):
        remainder = param.ds_numel % self.num_ranks_in_param_group
        return (self.num_ranks_in_param_group - remainder) if remainder else 0
    ...
    def _partition_param_sec(self, param, buffer=None, has_been_updated=False):
        ...
        if param.ds_status is ZeroParamStatus.AVAILABLE:
            #check padding
            tensor_size = self._aligned_size_sec(param)
