...
class RADIOEncoder(nn.Module):
    def __init__(self, config, delay_load=False):
        ...
        self.activation_multiplier = getattr(config, 'mm_vision_activation_multiplier', 1.)
    ...
    def load_model(self):
        ...
        for k, v in checkpoint_state_dict.items():
            if k not in model_state_dict:
                ...
                print(f"Checkpoint key {k} has shape {v.shape} but model state dict has shape {model_state_dict[k].shape}")

        # Only keep keys whose size matches that of the model parameters.
        checkpoint_state_dict = {k: v for k, v in checkpoint_state_dict.items() if k in model_state_dict and model_state_dict[k].shape == v.shape}
    ...
    def forward(self, x: torch.Tensor):
        ...
        print(f"features shape={features.shape} device={features.device} mean={features.mean().item()} std={features.std().item()}")
