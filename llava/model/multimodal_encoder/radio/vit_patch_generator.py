...
class ViTPatchGenerator(nn.Module):
    def __init__(
        ...
        if abs_pos and init_from is not None:
            patch_proj = init_from.patch_embed.proj
            self.embedder.weight.data.copy_(patch_proj.weight.flatten(1))
            self.patch_normalizer = init_from.norm_pre

            if init_from.cls_token is not None:
                init_cls_token = init_from.cls_token[:, 0].detach()
                if init_from.no_embed_class:
                    self._load_embed(init_from.pos_embed, self.pos_embed)
                else:
                    self._load_embed(init_from.pos_embed[:, 1:], self.pos_embed)
                    init_cls_token = init_cls_token + init_from.pos_embed[:, 0].detach()
                self.cls_token.init_from_cls_token(init_cls_token)
    ...
    def _get_pos_embeddings(self, batch_size: int, input_dims: Tuple[int, int]):
        ...
        if self.cpe_mode:
            if self.training:
                ...
                if indirect_grid_sample is not None:
                    indices = torch.zeros(batch_size, dtype=torch.int64, device=pos_embed.device)
                    pos_embed = indirect_grid_sample(
                        pos_embed,
                        grid=grid_xy,
