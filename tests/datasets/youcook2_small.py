...
class Panda70mTesting(unittest.TestCase):
    ...
    def test_dataloader_panda70m(self):
        ...
        workers = 2
        if osp.isdir("/lustre"):
            test_make_supervised_data_module(dataset_name="youcook2", batch_size=2, num_workers=workers, max_samples=50)
        elif osp.isdir("/mnt"):
            test_make_supervised_data_module(dataset_name="osmo_youcook2", batch_size=2, num_workers=workers, max_samples=50)
        else:
